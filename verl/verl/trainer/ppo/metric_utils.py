...
def compute_zoomin_metrics(batch: DataProto) -> dict[str, Any]:
    important_tokens = [151657]
    responses = batch.batch["responses"]
    metrics = dict()

    for important_token in important_tokens:
        token_counts_per_sample = (responses == important_token).sum(dim=1) / 2
        mean_count = token_counts_per_sample.float().mean().item()  

        metrics[f"critic/important_token_number_mean/{important_token}"] = mean_count

    print("compute_zoomin_metrics", metrics)
    return metrics
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    sequence_score_acc = torch.as_tensor(batch.non_tensor_batch["acc_score"], dtype=torch.float32)
    non_aborted_sequence_score_acc = sequence_score_acc[non_aborted_mask]

    acc_mean = non_aborted_sequence_score_acc.mean().item()
    acc_max = non_aborted_sequence_score_acc.max().item()
    acc_min = non_aborted_sequence_score_acc.min().item()
    acc_clip_ratio = (non_aborted_sequence_score_acc == max_response_length).float().mean().item()

    sequence_score_format = torch.as_tensor(batch.non_tensor_batch["format_score"], dtype=torch.float32)
    non_aborted_sequence_score_format = sequence_score_format[non_aborted_mask]

    format_mean = non_aborted_sequence_score_format.mean().item()
    format_max = non_aborted_sequence_score_format.max().item()
    format_min = non_aborted_sequence_score_format.min().item()
    format_clip_ratio = (non_aborted_sequence_score_format == max_response_length).float().mean().item()
    ...
    metrics = {
        ...
        "acc_reward/mean": acc_mean,
        "acc_reward/max": acc_max,
        "acc_reward/min": acc_min,
        "acc_reward/clip_ratio": acc_clip_ratio,
        "format_reward/mean": format_mean,
        "format_reward/max": format_max,
        "format_reward/min": format_min,
        "format_reward/clip_ratio": format_clip_ratio,
